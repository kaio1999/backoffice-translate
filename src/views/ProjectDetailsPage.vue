...
const allTranslations = ref([]); // Guarda todas as traduções
const translations = ref([]); // Traduções filtradas para exibição
const searchQuery = ref(''); // Termo de busca
...
const fetchLanguages = async () => {
  try {
    ...
    if (languages.value.length > 0 && !selectedLanguageCode.value) {
      selectedLanguageCode.value = languages.value[0].code; // Seleciona o primeiro idioma por padrão
    }
  } catch (error) {
...
const fetchTranslations = async () => {
  ...
  try {
    ...
    filterTranslations(); // Aplica o filtro inicial se houver busca
  } catch (error) {
...
// --- Funções de Manipulação de Traduções ---
...
onMounted(async () => {
  ...
  // fetchTranslations é chamado automaticamente pelo watcher ou seleção inicial
});
...
// Assista à mudança na pesquisa e filtre
